...
function getVideoAlphaMap(size, options = {}) {
    ...
    if (!alphaSource) {
        throw new Error("L'alpha map 96px de Gemini est manquante, impossible de générer le modèle de filigrane vidéo");
    }
...
export function detectDiamondVideoWatermarkFromFrames({
    ...
    if (!Array.isArray(frames) || frames.length === 0) {
        throw new Error("Aucune frame vidéo disponible pour la détection");
    }
    if (!candidates.length) {
        throw new Error(`Les candidats de filigrane vidéo en ${width}x${height} ne sont pas pris en charge pour le moment`);
    }
