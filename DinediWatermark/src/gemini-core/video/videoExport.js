...
function createRuntimeCanvas(width, height) {
    ...
    throw new Error("Aucun Canvas disponible dans cet environnement");
}
...
async function getVideoContext(file) {
    ...
    if (!videoTrack) {
        input.dispose();
        throw new Error("Aucune piste vidéo traitable dans le fichier");
    }
...
export async function detectGeminiVideoWatermark(file, options = {}) {
    ...
    try {
        ...
        if (!frames.length) {
            throw new Error("Impossible d'extraire les frames de détection de la vidéo");
        }
...
export async function removeGeminiVideoWatermark(file, options = {}) {
    ...
    if (!detection.isConfident && options.allowLowConfidence !== true) {
        throw new Error("Confiance de détection du watermark trop faible. Le traitement a été annulé.");
    }
    ...
    if (!canEncodeAvc) {
        throw new Error("Votre navigateur ne supporte pas l'encodage WebCodecs H.264/AVC. Utilisez une version récente de Chrome ou Edge.");
    }
    ...
    try {
        ...
        if (!target.buffer) {
            throw new Error("L'export vidéo a échoué, sortie vide");
        }
